...
public class MApplyServiceImpl extends MAbstractSearchServiceImpl implements MApplyService {
	...
	public Map<String, NetworkItem> getPowerNetwork(String artistName, int size) {
		...
		for (NetworkItem nextVisitItem : nextVisitItems ) {
			List<NetworkItem> nextNextVisitItemsP2 = visitCyclic(items, visitedArtists, items.get(nextVisitItem.getName()), size);
	...
	private List<NetworkItem> visitCyclic(Map<String, NetworkItem> items, Set<String> visitedArtists, NetworkItem rootItem, int size) {
	...
	private List<NetworkItem> getDependents(Map<String, NetworkItem> items, NetworkItem parentItem, int size, String type, String dslLocation, boolean isArtistSearch) {
		...
		if (root.getAsJsonObject().get("hits").getAsJsonObject().get("total").getAsInt() > 0 ) {
			jsonArray = root.getAsJsonObject().get("aggregations").getAsJsonObject().get(AGG_NAME).getAsJsonObject().get("buckets").getAsJsonArray();
			...
			for ( JsonElement jsonElement : jsonArray ) {
				if ( !jsonElement.getAsJsonObject().get("key").getAsString().equals(parentItem.getArtistName()) ) {
					if ( items.containsKey(jsonElement.getAsJsonObject().get("key").getAsString().replaceAll(" ", ""))) {
						dependentItem = items.get(jsonElement.getAsJsonObject().get("key").getAsString().replaceAll(" ", ""));
					} else {
