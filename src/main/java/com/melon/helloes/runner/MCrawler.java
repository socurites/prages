...
public class MCrawler {
	...
	public SongDoc crawlSongDetail(String songDetailUrl) {
		...
		if (!root.getAsJsonObject().get("lyricEntity").isJsonNull()
				&& !root.getAsJsonObject().get("lyricEntity").getAsJsonObject().get("LYRIC").isJsonNull()) {
			...
			List<String> genres = getGeneresAsList(
					songInfoObj.get("SONGGNRBASKET").getAsString().split("")[0].split("=")[1]);
			String issueDate = nullToBlank(songInfoObj.get("ISSUEDATE"));
			String labelName = nullToBlank(songInfoObj.get("PLANCNPY"));
			String lyric = lyricEntityObj.get("LYRIC").getAsString().replaceAll("<BR>", " ").replaceAll("<br>", " ")
			...
			if (!artistId.equals("2727")) {
				ArtistDoc artistDoc = this
						.crawlArtistDetail("http://www.melon.com/artist/timeline.json?artistId=" + artistId);
